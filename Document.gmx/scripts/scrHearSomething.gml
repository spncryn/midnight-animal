...
ds_list_sort(global.distlist, 1)
    ...
    if scrIgnoreSound()
    nothing = 1 else {
        if ds_list_find_value(global.distlist, 0) = dist and dist < 180 {
            if alert = 0 scrChasePlayer(path, argument0, argument1)
        }
        if ds_list_find_value(global.distlist, 1) = dist and dist < 180 {
            if alert = 0 scrChasePlayer(path, argument0, argument1)
        }
        if ds_list_find_value(global.distlist, 2) = dist and dist < 180 {
            if alert = 0 scrChasePlayer(path, argument0, argument1)
        }
    }
}

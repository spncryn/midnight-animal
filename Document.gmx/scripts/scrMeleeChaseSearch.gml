if checkreload > 0 checkreload -= 1 else {
    ...
    if check = 0 {
        path_end() alert = 1 speed = 2 direction = point_direction(x, y, argument0, argument1)
    }
    if check < 2 and check > 0 {
        if path_index >= 0 nothing = 1
        else {
            alert = 0 direction = round(direction * 0.1) * 10 speed = 1
        }
    }
    if path_position = 1 {
        if object_index=objEnemyDodger {
            scrChasePlayer(path, objPlayer.x, objPlayer.y)
            alert = 3
        } else {
            path_end()
            alert = 0

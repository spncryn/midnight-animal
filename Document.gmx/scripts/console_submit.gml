...
if console[? "active"] {
    ...
    if keyboard_check_pressed(vk_enter) && console[? "string"] != "" {
        ...
        if console[? "preset"] {
            switch(c[0]) {
                case "reset": game_restart(); break;
                case "end": game_end(); break;
                case "fullscreen": 
                    if no = 1 then
                ...
                case "create": 
                    if object_exists(asset_get_index(c[1])) {
                        ...
                        instance_create(global.mousex, global.mousey,asset_get_index(c[1]));
                    }
                break;
                case "screenshot": 
        ...
        show_debug_message("CONSOLE - Ran command: "+c[0]);

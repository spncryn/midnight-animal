///path_random(x1,y1,x2,y2,path,segment length,beam_tolerance,curl)
//By Ground Effect Games
//calculates the points needed for a lightning bolt and adds them to a path
//beam_tolerance is the maximum number of degrees a segment may vary from a straight line
//curl determines how many degrees the lightning's average path will curve.
var xx,yy,dir,t;
xx=argument0;
yy=argument1;
//beam_tolerance is limited in order to make the effect look nice and also use less CPU.
...
{
    dir=point_direction(xx,yy,argument2,argument3)+argument7+beam_tolerance(t);
    path_add_point(argument4,xx,yy,beam_tolerance(t*argument5/30));
